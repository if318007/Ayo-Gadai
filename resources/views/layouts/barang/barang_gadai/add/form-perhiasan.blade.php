        ...
        <div class="col-md-12">
            ...
            <form action="/data_barang_gadai/store/{{$kategori}}  " method="post" enctype="multipart/form-data">
                ...
                <div class="form-group {{$errors->has('jenis_kategori_id') ? 'has-error' : ''}}">
                    <label for="exampleFormControlInput1">Nasabah</label>
                            <div class="input-group">
                                ...
                                <input id="anggota_id" type="hidden" name="user_id" value="{{ old('anggota_id') }}" readonly="" required>
                ...
                <div class="form-group {{$errors->has('berat') ? 'has-error' : ''}}">
                    ...
                    <input type="number" class="form-control" id="exampleFormControlInput1" placeholder="Masukkan Berat Barang" name="berat" value="{{ old('berat') }}">
                    @if($errors->has('berat'))
                        <span class="help-block">Berat tidak boleh kosong!</span>
                    @endif
                </div>
                <div class="form-group {{$errors->has('total') ? 'has-error' : ''}}">
                    <label for="exampleFormControlInput1">Total</label>
                    <input type="number" class="form-control" id="exampleFormControlInput1" placeholder="Masukkan Total Barang" name="total" value="{{ old('total') }}">
                    @if($errors->has('total'))
                        <span class="help-block">Total tidak boleh kosong!</span>
                    @endif
                </div>
                ...
                <div class="form-group {{$errors->has('kondisi') ? 'has-error' : ''}}">
                    ...
                    <input type="text" class="form-control" id="exampleFormControlInput1" placeholder="Masukkan kondisi barang saat ini" name="kondisi" value="{{ old('kondisi') }}">
                    @if($errors->has('kondisi'))
                        <span class="help-block">Kondisi tidak boleh kosong!</span>
                    @endif
                </div>
                <div class="form-group {{$errors->has('kelengkapan') ? 'has-error' : ''}}">
                    <label for="exampleFormControlInput1">Kelengkapan</label>
                    <input type="text" class="form-control" id="exampleFormControlInput1" placeholder="Masukkan Jumlah Pinjaman" name="kelengkapan" value="{{ old('kelengkapan') }}">
                    @if($errors->has('kelengkapan'))
                        <span class="help-block">Kelengkapan tidak boleh kosong!</span>
                    @endif
                </div>
                <div class="form-group">
                    <label for="exampleInputEmail1">Keterangan</label>
                    <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" placeholder="Masukkan Keterangan Tambahan Barang (Optional), jika tidak ada ketikkan 'Tidak ada'" name="keterangan"></textarea>
                </div>
                ...
                <div class="form-group {{$errors->has('foto_file_barang') ? 'has-error' : ''}}">
                    <label for="exampleFormControlInput1">Unggah foto barang/ kelengkapan perhiasan (Lengkap)</label>
