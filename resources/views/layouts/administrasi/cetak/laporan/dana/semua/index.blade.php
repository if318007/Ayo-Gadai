...
<div style="padding-left:20px;">
  <center>
    <h1>Data Laporan Semua Aliran Dana</h1>
  </center>
                            ...
                            <tbody>
                                @foreach($laporan as $l)
                                    <tr>
                                        ...
                                        <td>{{$l->kas->nama_kas}} </td>
                                        <td>Rp. {{$l->jumlah}} </td>
                                        <td>{{$l->jenis_dana}} </td>
                                        <td>{{$l->keterangan}} Hari</td>
                                        <td>{{date('j F, Y', strtotime($l->tanggal))}}  </td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                        <p>Saldo Awal : Rp. {{$l->kas->saldo_awal}}</p>
                        <p>Total Dana Masuk : Rp. {{$dana_masuk}}</p>
                        <p>Total Dana Keluar : Rp. {{$dana_keluar-$dana_lunas}}</p>
                        <p>Saldo Akhir : Rp. {{$l->kas->saldo_akhir}}</p>
                </div>
            </div>
